...
app = Flask(__name__)
app.secret_key = os.urandom(24)  # Secret key for Flask session management

# --- Global AI Initialization (Instance) ---
AI_PLAYER = None  # Will hold the AI player instance
Q_TABLE_PATH = "connect4_q_agent_selfplay_gen2.pkl"  # Path to pre-trained Q-learning table
HYBRID_ITERATIONS = 1500  # Number of MCTS iterations per AI move (controls strength/thinking time)
...
try:
    ...
    AI_PLAYER = None  # Set to None if initialization fails

# Player colors for UI display
...
def update_game_state(**kwargs):
    """Updates the game state in the session with provided values."""
    state_changed = False
...
@app.route('/')
def index():
    """Main route that either shows player selection or game board based on state."""
    state = get_game_state()
    if not state['game_setup_complete']:
        return render_template('choose_player.html')  # Player selection screen
    else:
...
def start_game():
    ...
    if choice == 1: # Human chose P1 (Yellow, First)
        ...
        needs_ai_move = False
        message = f"Your turn ({PLAYER1_COLOR})" 
    elif choice == 2: # Human chose P2 (Red, Second)
        human_id = PLAYER2_PIECE
        ai_id = PLAYER1_PIECE
        initial_turn = ai_id
        needs_ai_move = True
        message = f"Starting Game... AI ({PLAYER1_COLOR}) is thinking..." 
    else:
        flash("Invalid player choice.", "error")
        return redirect(url_for('index'))

    clear_game_session()  # Clear any previous game state
    board_np = create_board()  # Create a fresh empty board
    update_game_state(
...
def ai_first_move():
    ...
    human_color = PLAYER2_COLOR   # Human is Player 2 (Red) in this scenario
    ...
    board_np = create_board()  # Create fresh board for AI's first move

    try:
        ai_start_time = time.time()
        # Set player IDs in the AI instance
        AI_PLAYER.player_id = ai_id
        AI_PLAYER.opponent_id = human_id
        col = AI_PLAYER.get_move(board_np.copy())  # Get AI's move
        ...
        if col is not None and is_valid_location(board_np, col):
            ...
            drop_piece(board_np, row, col, ai_id)  # Place AI's piece
            message = f"Your turn ({human_color})"
            turn = human_id  # Switch to human's turn
...
def play():
    ...
    col = data.get('column')  # Get column from request
    ...
    if col is None or not isinstance(col, int) or not is_valid_location(board_np, col):
        ...
        message = f"Invalid move (Column {col+1}?). Your turn ({human_color})." 
    ...
    if winning_move(board_np, human_id):
        message = f"Congratulations! You win! ({human_color})" 
    ...
    message = f"AI's turn ({ai_color})... Thinking..." 
...
def ai_move():
    ...
    try:
        ...
        AI_PLAYER.player_id = ai_id
        AI_PLAYER.opponent_id = human_id
        ai_col = AI_PLAYER.get_move(board_np.copy())  # Get AI's move decision
        ai_end_time = time.time()
        ...
        if ai_col is not None and is_valid_location(board_np, ai_col):
            ...
            drop_piece(board_np, ai_row, ai_col, ai_id)  # Place AI's piece

            # Check for AI win
            if winning_move(board_np, ai_id):
                message = f"AI Wins! ({ai_color})"
            ...
            return jsonify({
             ...
             message = f"Error: AI made invalid move. Your turn ({human_color})."
         ...
         message = f"Error during AI move. Your turn ({human_color})."
         update_game_state(turn=human_id, message=message)
